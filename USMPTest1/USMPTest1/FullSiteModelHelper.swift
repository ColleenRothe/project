...
class FullSiteModelHelper: NSObject, URLSessionDataDelegate{
    ...
    func parseJSON(){
        ...
        //FLMA LINK...spelled wrong!
        if(FDictionary.value(forKey:"FMLA_ID") as? String != nil){
            thing.flma_id = FDictionary.value(forKey: "FMLA_ID")! as? String}
        else{
            thing.flma_id = ""
        }
        
        if(FDictionary.value(forKey:"FMLA_NAME") as? String != nil){
            thing.flma_name = FDictionary.value(forKey: "FMLA_NAME")! as? String}
        else{
            thing.flma_name = ""
        }
        
        if(FDictionary.value(forKey:"FMLA_DESCRIPTION") as? String != nil){
            thing.flma_description = FDictionary.value(forKey: "FMLA_DESCRIPTION")! as? String}
        else{
